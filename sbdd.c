...
static struct block_device *target = NULL;
// for disk device
static char* disk_path=NULL;
module_param(disk_path,charp,S_IRUGO); 
static struct bio *sbdd_clone_bio_req(struct bio *bio)
{
	struct bio *cloned = bio_clone_fast(bio, GFP_NOIO, bio->bi_pool);
	if (!cloned){
...
static void forward_request(struct bio *bio_req){
	if (target != NULL){
		...
		bio_set_dev(cloned,target);
		submit_bio(cloned);
	}else{
		pr_err(" target block device isn't specified\n");
        bio_endio(bio_req);
	}
...
static int sbdd_create(void)
{
	...
	*/
	...
	if (disk_path == NULL){
		//default drive
		bdev = blkdev_get_by_path("/dev/sda",FMODE_READ | FMODE_WRITE,NULL);
	}else{
		bdev = blkdev_get_by_path(disk_path,FMODE_READ | FMODE_WRITE,NULL);
		
	}
	if (IS_ERR(bdev)){
		pr_err("can't get disk device");
	}
	// __sbdd_bdev_ops.owner = THIS_MODULE;
	 __sbdd.capacity = get_capacity(bdev->bd_disk);
	 __sbdd.q = bdev_get_queue(bdev);
	target = bdev;
...
static void __exit sbdd_exit(void)
{
	...
	sbdd_delete();
	if (target != NULL){
		blkdev_put(target, FMODE_READ | FMODE_WRITE );
	}
